...
const Card = ({pokemon, cardState}) => {
    ...
    const storePokemonLocal = () => {
        if (!localStorage.getItem('pokemonCatch')){
            ...
            setSave(true);
            console.log('all pokemon in local storage: ',JSON.parse(localStorage.getItem('pokemonCatch')));
    ...
    return (  
        <div className={"col-md-6 col-lg-4 mb-3"}>
            {/* card standard */}
            <div >
                <div className={"card m-0 border-dark btn hover-shadow"} onClick={()=>{setOpen(!open)}}>
            ...
            <div className={open ? "card-open btn" : "d-none"} style={setWrapperCardStyle()}>
                <div className={open ? "card-modal card m-0 border-dark btn" : "d-none"} >
                    <img className="card-img-top p-3" src={pokemon.sprites.other.dream_world.front_default} alt="pokemon sprite" style={{height: 15+'rem'}}></img>
                    <div className="card-body">
                        <h4 className="card-title">{pokemon.name.toUpperCase()}</h4>
                        <div className="card-type d-flex justify-content-center mb-2">
                        ...
                        {open &&<div className="card-info">
                            <button type="button" class="close close-button" aria-label="Close" onClick={()=>{setOpen(!open)}}>
